...
- `assets/css/app.css`: estilos nuevos para `.area-tab-group*`, se resalta el botón padre con `.group-active` cuando el área activa vive adentro. De paso se corrigió un bug latente: a `anticipos-recibidos`/`anticipos-entregados` les faltaba el color de fondo `.active` (quedaban con texto blanco invisible sobre fondo transparente al seleccionarlas) — se agregó `#0ea5e9` y `#f97316`.
...
**🔧 Fix (mismo día)**: Carlos reportó que el desplegable no se veía al hacer clic en "💰 Finanzas" — solo aparecía moviendo el scroll horizontal de la barra. Causa: `.area-tabs` tiene `overflow-x:auto`, y eso recorta verticalmente cualquier hijo `position:absolute` que se salga de su caja (el desplegable original). Corregido cambiando `.area-tab-dropdown` a `position:fixed`, con `top`/`left` calculados en JS (`toggleAreaDropdown()`, vía `getBoundingClientRect()` del botón) cada vez que se abre — así escapa el recorte sin importar scroll ni la barra `sticky`. Se cierra también al hacer scroll (con `position:fixed` no sigue al botón si la página se mueve).

**Archivos**: `tareas-equipo.html`, `assets/css/app.css` (`?v=20260914b`), `assets/js/tareas.js` (`?v=20260914c`), `assets/js/auth.js` (`?v=20260914a`).
...
Antes de este cambio, la pestaña "💰 Cartera" ya existía pero tenía tres límites que Carlos pidió resolver (después de revisar el trabajo de gestión de cartera que había hecho por su cuenta con Alegra/Gmail — archivo `estado_cartera.xlsx` en su carpeta de Reglas IA, con columnas Contactada/Fecha contacto/Etapa/Notas, y los correos "Cartera pendiente – Grupo Innovate S.A.S." que ya envía a mano): (1) los datos de Alegra estaban quemados en `ALEGRA_CARTERA_DATA` dentro de `cartera.js`, actualizados a mano por Claude cada vez que Carlos pedía "actualiza mi cartera"; (2) el estado del tablero (columna, responsable, notas, acuerdo) vivía solo en `localStorage` del navegador — no se compartía entre el equipo ni servía para un cron de recordatorios (y de hecho ni siquiera se recargaba al abrir la página, porque `loadCartera()` nunca se llamaba al iniciar la app — cada carga arrancaba con el tablero vacío hasta que `fetchCarteraAlegra()` lo repoblaba); (3) no había ningún envío de correo/WhatsApp, solo el tablero de seguimiento.

**Datos en vivo**: nuevo endpoint `backend/api/alegra_cartera_resumen.php` (solo admin) — pagina sobre `GET /invoices?status=open&dueDate_before=hoy` en Alegra, agrupa por cliente, y adjunta el `email`/`celular` que Ginno ya tenga guardado localmente para ese cliente (join por `alegra_id` contra la tabla `clientes`). Se consulta cada vez que se abre la pestaña — ya no hay botón de "dile a Claude que actualice".

**Estado del tablero en base de datos**: nueva tabla `cartera_gestion` (migración `db/040_cartera_gestion.sql`) — una fila por cliente (`cliente_alegra_id` como PK), con estado (mismas 5 columnas del kanban de siempre: por-contactar/llamado/correo/acuerdo/pagado), responsable, notas, fecha/monto de acuerdo, nivel de plantilla usado, fecha de último contacto y **próxima fecha de seguimiento**. Nuevo endpoint `backend/api/cartera_gestion.php` (GET lista todo, PUT hace upsert por cliente). La misma migración agrega `clientes.celular` (para WhatsApp) y el valor de configuración `cartera_dias_recordatorio` (días estándar que se sugieren al prellenar la próxima fecha de seguimiento — editable en ⚙️ Configuración, `assets/js/configuracion.js`, y se puede cambiar caso por caso al guardar cada gestión, tal como pidió Carlos).

**Envío de cobro**: `backend/lib/cartera_plantillas.php` genera el asunto y el texto del mensaje según el nivel elegido — 🙂 cordial (primer aviso, calcado del correo real que Carlos ya envía), 😐 firme (segundo aviso, más directo) o ⚠️ prejurídico (última instancia, avisa que sin respuesta en unos días se remite a cobro jurídico) — siempre con los mismos datos bancarios (Bancolombia) y firma que usa hoy. `backend/api/cartera_mensaje.php` arma la vista previa (con las facturas que ya trae el tablero, sin volver a consultar Alegra) y queda en un textarea editable en el modal antes de enviar — el mismo texto editado se usa para los dos canales. "Enviar por correo" (`backend/api/cartera_enviar_correo.php`) manda el texto (convertido a HTML simple con saltos de línea) y registra la gestión (fecha de contacto, nivel usado, próxima fecha de seguimiento). "Enviar por WhatsApp" abre un enlace `wa.me` con el mensaje precargado (Carlos revisa y da enviar desde su propio WhatsApp — no hay integración de API de WhatsApp, fue decisión explícita para la primera versión) y registra la gestión igual que el correo. El estado del tablero solo avanza automáticamente de "por-contactar" a "correo" — si ya estaba más adelante (llamado/acuerdo/pagado) no se toca.

**Recordatorio**: nuevo cron `backend/cron/cartera_recordatorio.php` (sugerido 8:00 a.m. diario, agregar en cPanel — ver DEPLOY.md) — revisa `cartera_gestion` por `fecha_proximo_seguimiento` vencida y `estado != 'pagado'`, y manda un correo consolidado a los administradores con la lista de clientes pendientes. Dedupe por día con el mismo patrón de `avisos_enviados` que ya usan los demás crons de Ginno (así si el cron corre más de una vez el mismo día no duplica el correo, pero si al día siguiente sigue vencido, vuelve a avisar).

**Archivos**: `backend/api/alegra_cartera_resumen.php`, `backend/api/cartera_gestion.php`, `backend/api/cartera_mensaje.php`, `backend/api/cartera_enviar_correo.php`, `backend/lib/cartera_plantillas.php`, `backend/cron/cartera_recordatorio.php` (todos nuevos) · `assets/js/cartera.js` (reescrito, ya no usa `ALEGRA_CARTERA_DATA` ni `localStorage`) · `backend/api/clientes.php` (campo `celular`) · `assets/js/tareas.js` (`setArea()` llama `fetchCartera()`) · `assets/js/configuracion.js` (nuevo campo `cartera_dias_recordatorio`) · `tareas-equipo.html` (modal de Cartera con los campos nuevos). `?v=20260912a` en `cartera.js`, `tareas.js`, `configuracion.js`. Migración `db/040_cartera_gestion.sql` debe correr antes del deploy.
