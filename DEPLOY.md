...
- `assets/js/ausencias.js` — vista de la pestaña. `?v=20260917b`.
...
**Archivo modificado:**
- `backend/api/reportes.php` — el `GET ?sin_reporte=1` ahora excluye tarjetas con `tipo_tarea = 'proyecto'`.

Sin cambios de frontend, sin migración. `backend/api/.` se copia completo por `.cpanel.yml`.

**Prueba manual sugerida:** con una tarjeta tipo Proyecto que tenga una visita cerrada sin reporte, verificar que NO aparece en la zona roja "Visitas terminadas sin reporte" del dashboard (solo admin) — sí debe seguir apareciendo cualquier tarjeta normal (Evento/Contrato) en la misma situación.
...
## Cambios pendientes de deploy (2026-09-08 — botón "Notificar cliente": resumen del estado actual por correo o WhatsApp)

`backend/api/notificar_cliente.php` (archivo nuevo — no requiere tocar `.cpanel.yml`, ya copia todo `backend/api/` con `cp -r`), `assets/js/tareas.js?v=20260908a` (requiere subir el `?v=` del script en `tareas-equipo.html`, ya hecho):

- **Pedido de Carlos**: un correo/WhatsApp para notificar al cliente con toda la info relevante que la tarjeta tenga hasta el momento — no solo el reporte de visita ya finalizada.
  - Alcance acordado con Carlos: botón solo en el modal de la tarjeta (no en el kanban), visible solo para admin, solo en tarjetas IT/IF con cliente identificado. Contenido del mensaje: datos básicos (cliente, título/servicio, estado actual en lenguaje amigable para el cliente, fecha/hora programada si aplica, técnico(s) asignado(s)) — sin horas de contrato, sin número de cotización/factura, sin la descripción interna de la tarjeta.
  - Como `clientes` no tiene número de WhatsApp guardado (solo `email`), el envío por WhatsApp usa el selector nativo de compartir del dispositivo (`navigator.share({text})`) — igual limitación/patrón que ya existía para compartir el PDF del reporte por WhatsApp; el técnico/admin elige el contacto desde su propio teléfono. En escritorio sin Web Share API, se copia el mensaje al portapapeles como respaldo.
  - `notificar_cliente.php`: GET `?tareaId=` devuelve el correo del cliente, un texto ya armado para WhatsApp y un resumen para previsualizar en el popup; POST `{tareaId, correos:[...]}` arma un correo HTML con la identidad de marca (mismo estilo que el recordatorio de visita del día anterior) y lo envía (con copia fija a administrativo@innovate.com.co, igual que el resto de correos al cliente). Etiquetas de estado traducidas a lenguaje de cliente (ej. "facturado" → "Servicio facturado"), no la jerga interna del kanban.
  - No queda registro/traza en BD de cuándo se envió esta notificación (a diferencia del reporte de visita, que sí actualiza `enviado_en`) — quedó fuera de alcance por ahora; si se necesita historial, sería una migración nueva.
...
- Antes, escribir o seleccionar (desde "🔍 Buscar en Alegra") un número de factura solo llenaba el campo `#f-factura` — mover la tarjeta a "Facturado" requería siempre un clic aparte en "✓ Marcar facturado".
- Ahora, en cuanto el campo de factura tiene un valor (por escritura manual o por selección en Alegra) y la tarjeta está en un estado donde aplica marcar como facturado (mismo criterio que ya usaba el botón), se llama automáticamente a `_marcarFacturadoDesdeModal()` — nueva función `_facturaCampoCambio()`.
...
- La barra de búsqueda del kanban (`getFiltered()` y el filtro de archivados) ahora también compara contra el ID de la tarjeta (`#XXXX`, primeros 4 caracteres). Funciona con o sin el `#` y con coincidencia parcial, igual que título/cliente/técnico.

## Cambios pendientes de deploy (2026-08-22 — más datos en el aviso de @mención)

Sin migraciones ni cron. `backend/api/comentarios.php`:

- El aviso de mención (correo y Telegram) ahora incluye cliente, ID de tarjeta (`#XXXX`, primeros 4 caracteres del id), fecha de ejecución y estado de la tarjeta, además del texto del comentario. Antes solo mostraba el título de la tarjeta y el texto.
- Nuevos helpers locales `_estadoLegible()` (traduce el estado interno a etiqueta en español) y `_fechaLegible()` (formatea `fecha_programacion` a d/m/Y).
